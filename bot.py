...
async def _update_progress(chat_id, msg_id, downloaded, total):
...
async def download_with_progress(message: Message):
    if message.document:
        ...
        orig = message.video.file_name or f"video_{message.id}.mp4"
    else:
        orig = f"file_{getattr(message, 'id', int(time.time()))}"
